...
const AdviseDetailes = () => {
  ...
  return (
    <div className={styles.main_advise}>
     ...
     <div className={styles.finger}> <FingerImage>
      ...
      <section className={styles.choosed_team}>
          <div className={styles.team_type}>
              <p onClick={myTeamHandler} className={teamType==="myTeam"?styles.active:null}>تیم های من</p>
            <p onClick={teamIncludedMeHandler} className={teamType==="teamIncludedMe"?styles.active:null}>تیم های که در آن عضو هستم</p>
          </div>
          {/* {} */}

          {teamType==="myTeam"?team?.map((item)=>{
             return <label className={styles.each_team}>
                  <div>
                      <img src={item.src} alt="عکس تیم"/>
                     <p>{item.teamName}</p>
                  </div>
                  <div>
                      <input type="radio" checked={selectedTeam.teamName===item.teamName} onChange={()=>selectedTeamHandler(item)} />
                  </div>
              </label>
